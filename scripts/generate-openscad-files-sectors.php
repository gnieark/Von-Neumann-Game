...
try {
    ...
    $outputs = [
        ...
        'scut' => generateSectorOpenScadOutputPath($root, $outputDir, $options['scutOutput'], 'scut-covered-sectors.scad'),
    ];
    ...
    $scut = generateSectorOpenScadQueryCoordinates(
        $pdo,
    ...
    generateSectorOpenScadWriteFile($outputs['scut'], generateSectorOpenScadContent('SCUT-covered sectors', $scut, $options['sphereDiameterMm']));
    ...
    echo '- SCUT-covered sectors: ' . count($scut) . ' -> ' . $outputs['scut'] . "\n";
    exit(0);
...
function generateSectorOpenScadParseArguments(array $argv): array
{
    ...
    foreach (array_slice($argv, 1) as $argument) {
        ...
        if (str_starts_with($argument, '--scut-output=')) {
            $options['scutOutput'] = generateSectorOpenScadNonEmptyPath(substr($argument, strlen('--scut-output=')), 'SCUT output');
            continue;
        }
...
function generateSectorOpenScadUsage(): string
{
    return <<<TEXT
Usage:
  php scripts/generate-openscad-files-sectors.php
...
function generateSectorOpenScadContent(string $title, array $coordinates, float $sphereDiameterMm): string
{
    ...
    $lines = [
        '// ' . $title,
        '// Generated by scripts/generate-openscad-files-sectors.php at ' . gmdate('c'),
