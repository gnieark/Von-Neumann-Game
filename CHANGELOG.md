...
- Contrôle Others « défense étoile — attente » : l’objectif logistique est porté à 30 auxiliaires et jusqu’à 20 auxiliaires embarqués peuvent participer simultanément à une moisson ; la réserve de ressources équivalente à 10 auxiliaires reste inchangée.
- Contrôle Others « défense étoile — attente » : les missiles visant le vaisseau mère déclenchent la défense centrale, même sans sonde ni Manny locale. Les autres vaisseaux de la flotte présents dans le secteur tentent une interception prioritaire, avec un seul missile par missile entrant et suivi des tirs pour éviter les doublons entre cycles. Les missiles visant d'autres vaisseaux sont ignorés par la défense centrale.
- Contrôle Others « défense étoile — attente » : quatre gardiens persistants par secteur de dépôt connu de la flotte, avec les réactions tactiques des sentinelles. Relève par un vaisseau disponible strictement mieux armé en missiles, retour de l'ancien gardien après l'arrivée du remplaçant, trajets par étapes et exclusion des navettes. Les gardiens restent aux dépôts lors des déménagements et poursuivent leur surveillance pendant le transit du vaisseau mère.
- Contrôle Others « défense étoile — attente » : déménagement automatique après épuisement des planètes locales. Choix prioritaire d’un voisin occupé moissonnable, puis exploration successive de la couronne à distance 2 par un vaisseau local ou une sentinelle ; retour sous 4 points de deutérium et reprise après le plein. Recherche et destination persistantes, attente des auxiliaires et navettes en cours, puis déplacement de toute la flotte et reprise de la formation après son arrivée complète.
...
- Contrôle Others « défense étoile — attente » : désengorgement automatique des cales pleines par construction de dépôt, dépôt local de la moitié des ressources disponibles ou navettes vers le dépôt connu le plus proche. Chargement proportionnel à la capacité libre, ravitaillement aller-retour, trajets par étapes, déchargement et retour suivis dans un journal persistant. Plusieurs navettes peuvent partir successivement et restent exclues de la formation défensive jusqu'à leur retour.
...
- Contrôle Others « défense étoile — attente » : les deux observations d'activité attribuable à une sonde sont désormais interrogées toutes les 20 secondes pour chaque sentinelle en poste, tandis que la réconciliation générale conserve son plafond de 300 secondes.
...
- API v123 / attente de stockage Manny : l’entrée en `waiting_for_space` expose désormais `task.waitingForSpaceSince`. Après sept jours continus sans capacité suffisante, la Manny abandonne automatiquement sa cargaison et retente son amarrage ; si son propre emplacement de 0,05 ECE reste indisponible, elle est détachée de sa sonde et devient un objet de secteur `abandoned` récupérable.
- Migration obligatoire avant redémarrage : scheduler arrêté, exécuter `php scripts/one-shot-scripts/migrate-manny-waiting-for-space-timeouts.php --database-config=…`. Comme l’ancien modèle ne conservait pas la date exacte d’entrée en attente, la migration initialise explicitement le délai des tâches existantes à son heure d’exécution.
- Mouvements sécurisés SCUT : lorsqu’un trajet relie deux secteurs possédant chacun un relais actif avec balise de transit dans le même réseau, la perte totale d’intégrité à l’arrivée est désormais plafonnée à 9 points de pourcentage, en complément de la protection existante contre la destruction à haute vélocité.
- Minage Manny : la récupération d’un container détaché rappelle désormais toutes les Mannys qui le minent, même si elles appartiennent à une autre sonde. Le déclenchement d’un astéroïde motorisé rappelle de la même manière les Mannys qui minent vers l’un de ses containers attachés, avec la raison `target_container_departed_with_asteroid`. La recherche s’appuie sur la projection SQL indexée `manny_tasks.target_container_id` et ne parcourt pas les événements du scheduler.
- Migration obligatoire avant redémarrage : exécuter `php scripts/one-shot-scripts/migrate-manny-target-container-index.php --database-config=…` afin de reconstruire la projection des containers cibles et de créer son index. Le script administratif `recall-failed-container-miners.php` permet en outre de rappeler explicitement, par joueur, les anciennes Mannys restées bloquées après la disparition de leur container cible.
- API v122 / détection des missiles : les scans courants de `GET /api/probe/sector` et `GET /api/probe/{probeId}/sector` exposent désormais chaque missile en vol depuis l’état canonique indexé de `others_projectiles`, sans parcourir les événements du scheduler. Le champ `targetsCurrentProbe` indique si la sonde observatrice est la cible déclarée. `/sensors` affiche alors une alerte rouge vivante avec le compte à rebours d’impact ; les autres missiles restent visibles dans la liste des objets sans déclencher ce panneau.
- Interface : la checklist « Préparation » de `/movement` indique désormais si l’intégrité minimale de 10 % est atteinte et bloque localement l’ordre de mouvement dans le cas contraire, en complément de la validation autoritaire de l’API.
- API v121 / intégrité des sondes : une sonde dont l’intégrité est strictement inférieure à 10 % ne peut plus préparer un déplacement (`probe_integrity_too_low`). Les dégâts sont plafonnés à l’intégrité restante et tout passage à 0 % place immédiatement la sonde au statut `dead`.
...
- Documentation OpenAPI : chaque opération possède désormais une catégorie fonctionnelle explicite, sans reliquat dans le groupe implicite `default`. Les 17 catégories sont déclarées dans leur ordre d’affichage Swagger, de `system` à `default probe`. Cette dernière explique l’équivalence avec les routes explicites portant un `probeId`, utilise automatiquement la sonde par défaut du joueur et reste volontairement isolée des catégories fonctionnelles. Les opérations canoniques transversales apparaissent aussi sous leurs catégories secondaires pertinentes : actions Manny sur les secteurs, inventaires, sondes, missiles, améliorations, SCUT et astéroïdes motorisés, avertissements sous `movement`, et partage de plans sous `scut`. Les opérations taguées sont définies directement, sans `$ref` d’opération susceptible de faire disparaître un endpoint lors de l’ouverture d’un accordéon Swagger. Les catégories Swagger sont repliées par défaut.
- API v118 / Others : ajout de la branche opérateur protégée par `can_control_others`, de l’idempotence globale au compte et de l’audit des commandes. Les flottes, vaisseaux, auxiliaires, actions, inventaires, transferts, déplacements, moissons, fabrications, lasers et missiles disposent d’identifiants opaques et n’exposent jamais de coordonnées absolues. Son contrat autonome est séparé dans `docs/openapi-others.yaml` et dispose de son interface Swagger sur `/api-docs-others` ; l’API principale reste documentée sur `/api-docs`.
- Gameplay Others : ajout des quatorze lots du modèle de faction, dans leur ordre de dépendance : persistance et scripts administratifs, scans BOB et unités autonomes, déplacements individuels/de flotte avec annulation, stocks planétaires canoniques, logistique, tâches auxiliaires et structures dormantes, moisson en essaim, ateliers, visibilité et alertes, laser, missiles partagés avec les sondes, dégâts et dissolution de flotte.
- Missiles : la recette Manny `missile` consomme un `battery_pack`, deux `micro_conductor`, 0,20 ECE de métaux et 0,10 ECE de composés carbonés en une heure pour une sortie de 0,05 ECE. Le lancement depuis une sonde réserve une Manny pendant une minute ; le lancement Others est planifié à échéance immédiate. Dans les deux cas, seuls les passages du scheduler créent puis résolvent le projectile.
- Secteurs : chaque planète possède désormais des `resourceAmounts` déterministes et le marqueur strict `harvestedByOthers`. Les API de scan n’en dérivent que des proportions par tranches de dix points. Les auxiliaires Others perdus ou détruits deviennent des structures persistantes inspectables et minables par les Mannys.
- Combat : les astéroïdes motorisés acceptent les vaisseaux mère et standards Others physiquement présents comme cibles. Un impact relativiste détruit le vaisseau ; la destruction du vaisseau mère dissout exclusivement sa flotte, retire ses standards et laisse les projectiles déjà lancés achever leur cycle autonome.
- Migration obligatoire avant redémarrage : exécuter `migrate-others-schema.php`, `migrate-others-integrity.php`, `migrate-others-combat-schema.php` puis `add-planet-resource-amounts.php` avec le bon `--database-config`. Les scripts refusent d’inventer un propriétaire ou de convertir implicitement des projectiles historiques.
...
- API v114 : chaque alerte retournée par `GET /api/probe/alerts` et ses variantes ciblant une sonde expose désormais `illustrationImageUrl`, URL HTTP(S) absolue nullable. Avant de redémarrer l’application sur une base existante, exécuter puis rejouer `php scripts/one-shot-scripts/migrate-alert-illustration-images.php --database-config=config/database.json` ; le second passage doit annoncer `column_added=no`.
- Interface : `/alerts` affiche l’éventuelle illustration sous le texte de l’alerte, sur toute la largeur intérieure de la carte, en conservant son ratio d’origine.
...
- Interface : l’aide du formulaire « Lancer un astéroïde motorisé » s’adapte désormais au mode choisi et explique séparément l’impact dans le système ou le transfert intersectoriel avec sa probabilité décroissante de capture.
...
- Correction WebUI : le prérequis de motorisation mentionne et vérifie désormais l’intégralité du coût — un moteur au deutérium, quatre barres d’acier, deux plaques d’acier et 0,2 point de deutérium.
...
### Changed
- API : `GET /api/probe/{probeId}/mannies` et `GET /api/probe/{probeId}/mannies/{mannyId}` exposent désormais `taskStartTime` (ISO-8601, nullable) avec les détails des tâches Manny.
...
### Changed
- API v106 : le minage Manny ne crée plus d'étapes intermédiaires dans le scheduler. La durée complète (trajets compris) est calculée au lancement, puis l'extraction et le dépôt sont transactionnés à l'échéance finale, après vérification de la place disponible.
- Migration : ajout de `scripts/migrate-mining-to-terminal-events.php` pour convertir, worker arrêté, les minages actifs vers leur unique événement terminal et restituer aux astéroïdes les cargaisons encore en vol.
...
- Interface : `/mannies` borne désormais son polling à une requête groupée toutes les 5 secondes et temporise les transitions scheduler échues, afin d’éviter une boucle rapide menant au statut HTTP 429 lorsque le worker tarde à persister une transition.
...
- Performance : l’affectation atomique d’un lot de tâches Manny rafraîchit désormais les tâches existantes une seule fois et réutilise le verrou de la sonde, au lieu de rescanner toutes les Mannies pour chaque ordre.
...
- API v99 : les observations de secteur exposent `scutCoverageStatus` (`covered`, `uncovered` ou `unknown`) et ne révèlent sur un secteur non visité que les réseaux SCUT dont le joueur a déjà visité au moins un secteur couvert.
...
- Interface : le formulaire de craft `/mannies` détaille désormais les sous-composants automatiquement préparés pour les recettes profondes comme Manny, afin de rendre visibles les barres, plaques et autres pièces nécessaires aux composants intermédiaires.
- Interface : quand `/mannies` mine uniquement du deutérium, le champ de quantité utilise désormais des points de cuve; la requête garde l’unité moteur en ECE avec la conversion 1 point = 0.01 ECE minée.
...
- Maintenance : ajout de `scripts/generate-threejs-point-cloud-sectors.php` pour exporter les secteurs générés, visités, occupés par une sonde et couverts par SCUT en nuages de points JSON chargeables par Three.js, dont une scène `sector-point-clouds-threejs-editor.json` importable dans l’éditeur Three.js.
...
- Stockage des items : remplacement de `probe_items.metadata_json` et `detached_storage_container_items.metadata_json` par des colonnes relationnelles pour la fabrication, le bonus de capacité et la source de restauration; seules les métadonnées administratives rares restent dans `audit_metadata_json`. Exécuter `php scripts/one-shot-scripts/migrate-item-metadata-storage.php --database-config=var/database-prod.json` avant de déployer le nouveau code.
