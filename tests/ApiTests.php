...
use VonNeumannGame\FrontRoute\FrontRouteInventories;
use VonNeumannGame\FrontRoute\FrontRouteMovement;
use VonNeumannGame\FrontRoute\FrontRouteSectorStorage;
...
$test->assert(
    is_string($mainScript)
        && str_contains($mainScript, 'if (response.status === 401)')
        && str_contains($mainScript, 'sessionCheckPromise = fetch("/api/me"')
        && str_contains($mainScript, 'window.location.reload()'),
    'WebUI checks the current session once and reloads after an authentication failure'
);
...
$renderedSectorStorageView = (new FrontRouteSectorStorage())->getContent('GET', '/sector-storage', 'test-bearer', 'fr');
$inventoriesRoute = new FrontRouteInventories();
$inventoriesRoute->configureRequestContext('/inventories/42', 42);
$renderedInventoriesView = $inventoriesRoute->getContent('GET', '/inventories/42', 'test-bearer', 'fr');
...
$test->assert(str_contains($renderedSectorStorageView, '<h2>Stockages dans le secteur</h2>'), 'sector-storage route renders its French page title');
$test->assert(str_contains($renderedSectorStorageView, 'id="sector-storage-select"'), 'sector-storage route renders the storage selector');
$test->assert(str_contains($renderedInventoriesView, 'class="button-link secondary-button-link"'), 'inventories route renders sector storage as a secondary header action');
$test->assert(str_contains($renderedInventoriesView, 'href="/sector-storage/42"'), 'inventories sector-storage link preserves the selected probe');
$test->assert(str_contains($renderedInventoriesView, '>Stockages du secteur</a>'), 'inventories route labels its sector-storage link in French');
...
$sectorStorageScript = file_get_contents($root . '/public/assets/sector-storage.js');
$sectorStorageTemplate = file_get_contents($root . '/templates/sector-storage.html');
$sectorStorageRoute = file_get_contents($root . '/src/FrontRoute/FrontRouteSectorStorage.php');
...
$forumTemplate = file_get_contents($root . '/templates/forum.html');
...
$test->assert(is_string($frontIndex) && preg_match('/"SectorStorage"\s*=>\s*\[.*?\'displayOnMainMenu\'\s*=>\s*false/s', $frontIndex) === 1, 'sector-storage front route stays out of the left navigation');
$test->assert(is_string($frontIndex) && str_contains($frontIndex, "'#^/sector-storage(?:/\\d+)?$#'"), 'sector-storage front route accepts an optional probe id');
$test->assert(is_string($mainScript) && str_contains($mainScript, 'inventories|sector-storage|mannies'), 'main JS keeps the selected probe while browsing sector storage');
...
$test->assert(
    str_contains($openApi, 'guaranteed in the first entered sector')
        && str_contains($openApi, 'reduces the next capture probability by')
        && str_contains($openApi, '10 percentage points (100%, 90%, 80%')
        && str_contains($openApi, 'Black holes always capture the asteroid.'),
    'OpenAPI explains sector-transfer capture probability',
);
...
$test->assert(is_string($translatorSource) && str_contains($translatorSource, "'forumBriefTitle' => 'Note de service n° NP-43 (abroge 42-BIS)'"), 'French translations include the updated forum notice title');
$test->assert(is_string($translatorSource) && str_contains($translatorSource, "'forumBriefTitle' => 'Service memo no. NP-43 (supersedes 42-BIS)'"), 'English translations include the updated forum notice title');
$test->assert(is_string($translatorSource) && str_contains($translatorSource, "'forumDiscordServer' => 'serveur Discord de Neumann Probe'"), 'French translations include the Discord server label');
...
$test->assert(is_string($inventoriesScript) && str_contains($inventoriesScript, 'function shouldPreserveInventoryLineForm()'), 'inventories JS detects an active inline inventory action form');
$test->assert(is_string($inventoriesScript) && str_contains($inventoriesScript, 'if (shouldPreserveInventoryLineForm())'), 'inventories refresh preserves an actively edited move or detach form');
...
$test->assert(is_string($manniesScript) && str_contains($manniesScript, 'explicitCurrentProbeApiPath("/sector-objects/"'), 'Manny storage forms read external inventories through an explicit probe route');
$test->assert(is_string($manniesScript) && str_contains($manniesScript, 'explicitCurrentProbeApiPath("/mannies/" + encodeURIComponent(mannyId) + "/storage-transfers")'), 'Manny storage forms submit transfers through an explicit probe route');
$test->assert(is_string($manniesScript) && str_contains($manniesScript, 'explicitCurrentProbeApiPath("/mannies/" + encodeURIComponent(mannyId) + "/transfer-deuterium-from-external-storage")'), 'Manny refueling submits through an explicit probe route');
...
$test->assert(is_string($translatorSource) && str_contains($translatorSource, "'asteroidLaunchSectorTransferHint' => 'Le secteur voisin choisi"), 'French translations explain sector-transfer capture probability');
$test->assert(is_string($appCss) && str_contains($appCss, '.manny-asteroid-launch-hint'), 'Manny CSS styles asteroid launch help');
...
$test->assert(is_string($manniesTemplate) && str_contains($manniesTemplate, 'id="manny-filter-status"'), 'mannies view exposes a status filter');
$test->assert(is_string($manniesTemplate) && str_contains($manniesTemplate, 'id="manny-filter-range"'), 'mannies view exposes a range filter');
$test->assert(is_string($manniesTemplate) && str_contains($manniesTemplate, 'id="manny-sort-order"'), 'mannies view exposes a sort selector');
$test->assert(is_string($manniesTemplate) && str_contains($manniesTemplate, 'id="manny-filter-reset"'), 'mannies view exposes a filter reset control');
$test->assert(is_string($manniesScript) && str_contains($manniesScript, 'renderVisibleMannies'), 'mannies JS renders the filtered Manny list');
$test->assert(is_string($manniesScript) && str_contains($manniesScript, 'mannyMatchesFilters'), 'mannies JS filters Mannys before rendering');
$test->assert(is_string($manniesScript) && str_contains($manniesScript, 'sortMannies'), 'mannies JS sorts Mannys before rendering');
...
$test->assert(is_string($appCss) && str_contains($appCss, '.manny-filter-row'), 'mannies CSS styles the filter row');
...
$test->assert(is_string($translatorSource) && str_contains($translatorSource, "'ingredientAvailabilityAfterIntermediateComponents' => 'Disponible après déduction des composants affectés aux ingrédients intermédiaires précédents.'"), 'French translations explain availability adjusted by intermediate components');
$test->assert(is_string($translatorSource) && str_contains($translatorSource, "'ingredientAvailabilityAfterIntermediateComponents' => 'Available after components allocated to preceding intermediate ingredients.'"), 'English translations explain availability adjusted by intermediate components');
...
if ($createdProbe !== null) {
    ...
    if ($cliInventoryProbe !== null) {
        ...
        $test->assert(str_contains($addInventoryItemText, 'probe #' . $cliInventoryProbe->id), 'add-inventory-item CLI reports the targeted probe');
        ...
        $addInventoryAuxProbeCommand = escapeshellarg(PHP_BINARY)
            . ' ' . escapeshellarg($root . '/scripts/add-inventory-item.php')
            . ' --database-config=' . escapeshellarg($userinfosDbConfig)
            . ' --object=' . escapeshellarg(ProbeItem::TYPE_MICRO_CONDUCTOR)
            . ' --quantity=1'
            . ' --probe-id=' . $cliInventoryAuxProbe->id;
        exec($addInventoryAuxProbeCommand . ' 2>&1', $addInventoryAuxProbeOutput, $addInventoryAuxProbeStatus);
        $test->assertEquals(0, $addInventoryAuxProbeStatus, 'add-inventory-item CLI exits successfully for explicit probe targets');
        $test->assert(str_contains(implode("\n", $addInventoryAuxProbeOutput), 'probe #' . $cliInventoryAuxProbe->id), 'add-inventory-item CLI reports the explicit probe target');
        $cliAuxConductors = array_values(array_filter(
            $items->findByProbeId($cliInventoryAuxProbe->id),
            static fn(ProbeItem $item): bool => $item->type === ProbeItem::TYPE_MICRO_CONDUCTOR,
        ));
        $cliDefaultConductors = array_values(array_filter(
            $items->findByProbeId($cliInventoryProbe->id),
            static fn(ProbeItem $item): bool => $item->type === ProbeItem::TYPE_MICRO_CONDUCTOR,
        ));
...
$freshSessionLastUsedAt = gmdate('c');
$setSessionLastUsed->execute(['last_used_at' => $freshSessionLastUsedAt, 'hash' => $sessionTouchHash]);
$freshSessionTouch = $kernel->handle('GET', '/api/me', $headers);
$test->assertEquals(200, $freshSessionTouch->status, 'valid token remains accepted when session touch is throttled');
$getSessionLastUsed->execute(['hash' => $sessionTouchHash]);
$test->assertEquals($freshSessionLastUsedAt, $getSessionLastUsed->fetchColumn(), 'session last_used_at is not rewritten within five minutes');
$staleSessionLastUsedAt = gmdate('c', time() - 301);
$setSessionLastUsed->execute(['last_used_at' => $staleSessionLastUsedAt, 'hash' => $sessionTouchHash]);
$staleSessionTouch = $kernel->handle('GET', '/api/me', $headers);
$test->assertEquals(200, $staleSessionTouch->status, 'valid token remains accepted when session touch is stale');
$getSessionLastUsed->execute(['hash' => $sessionTouchHash]);
$test->assert($getSessionLastUsed->fetchColumn() !== $staleSessionLastUsedAt, 'session last_used_at is refreshed after five minutes');
...
$apiKeyTouchHash = ApiKeyRepository::hashToken((string) $apiKeyToken);
$getApiKeyLastUsed = $pdo->prepare('SELECT last_used_at FROM api_keys WHERE token_hash = :hash');
$getApiKeyLastUsed->execute(['hash' => $apiKeyTouchHash]);
$test->assert($getApiKeyLastUsed->fetchColumn() !== null, 'API key first use stores last_used_at');
$setApiKeyLastUsed = $pdo->prepare('UPDATE api_keys SET last_used_at = :last_used_at WHERE token_hash = :hash');
$freshApiKeyLastUsedAt = gmdate('c');
$setApiKeyLastUsed->execute(['last_used_at' => $freshApiKeyLastUsedAt, 'hash' => $apiKeyTouchHash]);
$freshApiKeyTouch = $kernel->handle('GET', '/api/me', ['Authorization' => 'Bearer ' . $apiKeyToken]);
$test->assertEquals(200, $freshApiKeyTouch->status, 'API key remains accepted when touch is throttled');
$getApiKeyLastUsed->execute(['hash' => $apiKeyTouchHash]);
$test->assertEquals($freshApiKeyLastUsedAt, $getApiKeyLastUsed->fetchColumn(), 'API key last_used_at is not rewritten within five minutes');
$staleApiKeyLastUsedAt = gmdate('c', time() - 301);
$setApiKeyLastUsed->execute(['last_used_at' => $staleApiKeyLastUsedAt, 'hash' => $apiKeyTouchHash]);
$staleApiKeyTouch = $kernel->handle('GET', '/api/me', ['Authorization' => 'Bearer ' . $apiKeyToken]);
$test->assertEquals(200, $staleApiKeyTouch->status, 'API key remains accepted when touch is stale');
$getApiKeyLastUsed->execute(['hash' => $apiKeyTouchHash]);
$test->assert($getApiKeyLastUsed->fetchColumn() !== $staleApiKeyLastUsedAt, 'API key last_used_at is refreshed after five minutes');
