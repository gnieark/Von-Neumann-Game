...
(static function () use ($test,$pdo,$players,$probes,$auth,$others,$kernel,$sectorRepository,$germinationDepots,$storageTransfers,$storage,$mannies,$mannyStorageTransfers,$mannyService): void {
    ...
    $knownDepotsPath = '/api/others/fleets/' . $fleet['public_id'] . '/known-depots';
    $test->assertEquals(['knownDepots' => []], $kernel->handle('GET', $knownDepotsPath, $headers)->body, 'starting construction does not discover an unfinished depot');
    $germinationDepots->completeConstruction((int) $action['id'], (new DateTimeImmutable($action['ends_at']))->modify('-1 second')->format('c'));
    $test->assertEquals(['knownDepots' => []], $kernel->handle('GET', $knownDepotsPath, $headers)->body, 'early construction processing does not discover a depot');
    $germinationDepots->completeConstruction((int)$action['id'],$action['ends_at']);
    $expectedKnownDepots = ['knownDepots' => [['relativeCoordinates' => ['x' => 0, 'y' => 0, 'z' => 0]]]];
    $test->assertEquals($expectedKnownDepots, $kernel->handle('GET', $knownDepotsPath, $headers)->body, 'completed HTTP construction reveals the depot sector to its fleet in owner-relative coordinates');
    $germinationDepots->completeConstruction((int) $action['id'], $action['ends_at']);
    $test->assertEquals($expectedKnownDepots, $kernel->handle('GET', $knownDepotsPath, $headers)->body, 'construction completion replay keeps one known depot sector');

    $otherFleet = $others->createFleet($player->id, 315, 7, -8);
    $otherShip = $others->findShipsByFleetId((int) $otherFleet['id'])[0];
    $otherAux = $others->createAuxiliary((int) $otherShip['id']);
    $otherKnownDepotsPath = '/api/others/fleets/' . $otherFleet['public_id'] . '/known-depots';
    $test->assertEquals(['knownDepots' => []], $kernel->handle('GET', $otherKnownDepotsPath, $headers)->body, 'construction discovery is not shared with another fleet of the same owner in the sector');
    $pdo->prepare("UPDATE others_inventory_resources SET amount=2 WHERE ship_id=? AND resource_type='metals'")->execute([$otherShip['id']]);
    $otherBuildPath = '/api/others/ships/' . $otherShip['public_id'] . '/auxiliaries/' . $otherAux['public_id'] . '/build-germination-depot';
    $interruptedBuild = $kernel->handle('POST', $otherBuildPath, $headers, '{}');
    $test->assertEquals(202, $interruptedBuild->status, 'another fleet can start a depot construction');
    $interruptedAction = $others->findActionByPublicId($interruptedBuild->body['action']['id']);
    $interruptedAt = (new DateTimeImmutable($interruptedAction['ends_at']))->modify('-1 second')->format('c');
    $germinationDepots->completeConstruction((int) $interruptedAction['id'], $interruptedAt, 'auxiliary_destroyed');
    $germinationDepots->completeConstruction((int) $interruptedAction['id'], $interruptedAction['ends_at']);
    $test->assertEquals(['knownDepots' => []], $kernel->handle('GET', $otherKnownDepotsPath, $headers)->body, 'interrupted construction and delayed replay do not discover even an existing depot in the sector');
    $others->deleteFleetByMothershipPublicId((string) $otherShip['public_id']);
