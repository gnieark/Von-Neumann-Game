...
(static function ($test): void {
    ...
    $transaction=new StorageTransaction($db);$locks=new StorageLockRepository($db);$transferRows=new SectorStorageTransferRepository($db);$inventoryTransfers=new InventoryTransferRepositoryFactory($db);$detached=new DetachedStorageContainerRepository($db);
    $effects = new SectorEffectService(new SectorEffectRepository($db),$events,$sectors);
    $waves = new AnomalyBroadcastService(new AnomalyBroadcastRepository($db),$transaction,$locks,$events,new OthersAuditRepository($db));
    $service = new GerminationDepotService($others,$events,$sectors,$effects,$waves,$transaction,$locks,new StorageActorRepository($db),$depots,$detached,$clock);
    $transfers = new SectorStorageTransferService($transaction,$locks,$transferRows,$inventoryTransfers,$depots,$service,$clock);
    $db->beginTransaction();
    $effects->enqueue('rolled-back-effect',$coordinates,'consume_object','never-published',[],$now->format('c'));
    $db->rollBack();
    $test->assertEquals(0,(int)$db->query("SELECT COUNT(*) FROM sector_effects WHERE operation_id='rolled-back-effect'")->fetchColumn(),'rollback before commit leaves no sector effect intention');
    $test->assertEquals(0,(int)$db->query("SELECT COUNT(*) FROM scheduled_events WHERE type='sector.effect'")->fetchColumn(),'rollback before commit leaves no sector effect event');
    ...
    $mannyTransfers=new \VonNeumannGame\Service\MannyStorageTransferService($transaction,$locks,$transferRows,new StorageReservationRepository($db),$inventoryTransfers,$depots,$detached,$containers,$mannies,$probes,$storage,[], $clock);
