...
final class MannyService implements MannyTaskRuntime
{
    ...
    public function __construct(
        ...
        $this->motorizeAsteroidTaskHandler = new MotorizeAsteroidTaskHandler(
            ...
            fn(mixed $sectorCoordinates): SectorContent => $this->sectors->getOrCreateSector($sectorCoordinates),
            fn(NeumannProbe $probe): array => $this->items->findByProbeId($probe->id),
    ...
    public function mannyForProbeApi(NeumannProbe $probe, string $uid): Manny
    {
        $this->storage->ensureProbeStorage($probe);
    ...
    public function startMotorizingAsteroid(NeumannProbe $probe, string $uid, string $objectId): Manny
    {
        return $this->withProbeLock(
            $probe,
            function (NeumannProbe $lockedProbe) use ($uid, $objectId): Manny {
                if ($this->improvements?->findForProbe($lockedProbe->id, ProbeImprovementCatalog::DISTRIBUTED_THRUST_ANCHORING)?->available !== true) {
                    throw new MannyActionException(422, 'distributed_thrust_anchoring_unavailable', 'The Distributed Thrust Anchoring blueprint has not been unlocked.');
                }
    ...
    {
        $completed = $this->refreshStorageWaitTimeout($manny, $probe, $now);
        if ($completed !== null) {
            return $completed;
        }
    ...
    public function refreshStorageWaitTimeout(Manny $manny, NeumannProbe $probe, \DateTimeImmutable $now): ?Manny
    {
        ...
        try {
            ...
            throw new \RuntimeException('Storage wait has an invalid canonical start timestamp.', previous: $error);
        }
        if ($now < $timeoutAt) {
            return null;
        }
        ...
        if ($manny->isInSameSectorAs($probe) && $this->storage->placeMannyOnProbe($probe, $manny)) {
    ...
    private function requiredWaitingForSpaceSince(Manny $manny): string
    {
        ...
        if (!is_string($waitingSince) || trim($waitingSince) === '') {
            throw new \RuntimeException('Storage wait is missing its canonical start timestamp; run the migration for waiting-for-space or blocked mining tasks.');
        }
    ...
    private function dormantConstructReportMessage(string $scenario): string
    {
        return match ($scenario) {
            ...
            ProbeImprovementCatalog::DISTRIBUTED_THRUST_ANCHORING => "Manny report\n\n"
