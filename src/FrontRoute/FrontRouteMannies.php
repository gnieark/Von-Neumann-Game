...
class FrontRouteMannies extends FrontRoute{
    ...
    public function getContent(string $method, string $routePath, ?string $bearer, string $language): string
    {
        ...
        return $tpl->applyTplFile($projectRoot . '/templates/mannies.html');
    }
    ...
    public function getPageTitle(?string $bearer, string $language): string
    {
        ...
        return 'Von Neumann Game - ' . $translator->get('tabMannies');
    }

    public function getCustomJs(): string
    {
        return '<script src="/assets/mannies.js?v=' . (defined('ASSET_VERSION') ? ASSET_VERSION : '') .'" defer></script>';
    }
