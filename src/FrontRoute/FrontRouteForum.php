...
class FrontRouteForum extends FrontRoute{
    ...
    public function getContent(string $method, string $routePath, ?string $bearer, string $language): string
    {
        ...
        return $tpl->applyTplFile($projectRoot . '/templates/forum.html');
    }
    ...
    public function getPageTitle(?string $bearer, string $language): string
    {
        ...
        return 'Von Neumann Game - ' . $translator->get('tabForum');
    }
    ...
    {
        return '<script src="/assets/forum.js?v=' . (defined('ASSET_VERSION') ? ASSET_VERSION : '') .'" defer></script>';
    }
}
