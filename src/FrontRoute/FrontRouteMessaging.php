...
class FrontRouteMessaging extends FrontRoute{
    ...
    public function getContent(string $method, string $routePath, ?string $bearer, string $language): string
    {
        ...
        return $tpl->applyTplFile($projectRoot . '/templates/messaging.html');
    }
    ...
    public function getPageTitle(?string $bearer, string $language): string
    {
        ...
        return 'Von Neumann Game - ' . $translator->get('tabMessages');
    }

    public function getCustomJs(): string
    {
        return '<script src="/assets/messaging.js?v=' . (defined('ASSET_VERSION') ? ASSET_VERSION : '') .'" defer></script>';
    }
